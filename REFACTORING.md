...
- **実施済み**: #1〜#31 のすべて（#32 を除く全項目）
  - #19 は Pinia 導入までは行わず、`ref(props.x)` を `toRef(props, 'x')` に統一し「親リストとオブジェクトを共有し deep watch で永続化する」設計意図をコメントで明文化する対応とした
  - #20 はリスト項目に `id` を付与（追加時に採番、既存データはロード時に `ensureIds` で補完）
  - #30 はノーマライズ結果を `_normalized` 付き別名で保存する仕様に変更（元ファイルを上書きしない）
